...
`markets-service` is served from `https://api.numofx.com`. The examples below use
`$MARKETS_SERVICE_URL`, so export it once:

```bash
export MARKETS_SERVICE_URL="https://api.numofx.com"
```

Read endpoints are public — no API key, and no allowlisting for server-side clients.

<Note>
  Integrating against USDC/cNGN **spot** rather than futures? Its prices are reported in two
  different units, and the book's `bids` and `asks` are named for the engine rather than the
  trader. Start with the [Spot market](/spot) guide.
</Note>
...
```bash
curl "$MARKETS_SERVICE_URL/v1/book?symbol=USDCcNGN-SEP16-2026"
curl "$MARKETS_SERVICE_URL/v1/trades?symbol=USDCcNGN-SEP16-2026&limit=50"
```
...
Orders are posted to `markets-service`, but the request must already contain a signed action payload that matches the contracts. See [Authentication and signing](/signing) for how to build `action_json.data` and produce the signature.
...
<Warning>
  `desired_amount` and `limit_price` in the body are **human decimals** — `markets-service`
  normalizes them against the instrument's minimum size and tick size. The wei values go in the
  signed `action_json.data`. See [Authentication and signing](/signing#price-and-amount-use-two-different-scales).
</Warning>
...
For the `USDCcNGN-SEP16-2026` future:
