...
| `limitPrice`    | `int256`  | `limit_price`    |
| `desiredAmount` | `int256`  | `desired_amount` |
| `worstFee`      | `uint256` | `worst_fee`      |
...
const asset = '0xDd9c2Ddf...A1F9'      // from GET /v1/markets
const subId = 1789516800n              // from GET /v1/markets
...
- `data.limitPrice` is a positive whole multiple of the normalized `limit_price`
- `data.desiredAmount` is a positive whole multiple of the normalized `desired_amount`

<Warning>
  The price and amount checks compare the onchain-scale integers in `data` against the
  normalized values in the flat fields. Build both from the same source numbers rather than
  rounding them independently, or the divisibility check fails.
</Warning>
